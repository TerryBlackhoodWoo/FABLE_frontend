...
질문을 입력하면 백엔드(`FastAPI`)의 `/ask`를 호출해, 신화·역사 속 인물(호메로스·헤로도토스 또는 핸드오프된 캐릭터)의 답변과 원전 출처(작품·챕터·유사도 점수), 그리고 그 장면을 그린 고전 미술/유물 이미지를 함께 보여주는 채팅 UI입니다. 대화는 배열로 누적되어 멀티턴으로 이어집니다.
...
| 경로 | 설명 | 인증 방식 |
|---|---|---|
| `/` | 메인 채팅 화면 (멀티턴) | 랜딩은 공개, 질문 제출 시 로그인 여부 확인 |
| `/login` | 대시보드 로그인 | — |
| `/logs` | 대화 로그·사용량 조회 | `Authorization: Bearer <JWT>` (localStorage) |
| `/admin` | 관리자 페이지 (계정 생성/권한/로그인 설정/비활성화, 전체 이력) | `Authorization: Bearer <JWT>`, `is_admin` 계정만 |
...
│   ├── page.tsx           # 메인 채팅 UI ("/") — 멀티턴 대화 배열 누적 표시
│   ├── login/
...
└── lib/
    └── useAuth.ts           # 인증 상태(로그인/관리자 여부) + 로그아웃 공용 훅

public/
└── fable-title.png      # FABLE 워드마크 로고 이미지
```

## 기능

- 질문 입력 → `X-API-Key` 헤더와 함께 `/ask` 호출 → 화자·답변·원전 출처·장면 이미지 표시
- 대화가 배열로 누적 표시되는 멀티턴 UI — 각 턴마다 답변 아래 원전 근거(작품·챕터·유사도 점수) 목록 노출
- 로그인 → JWT를 `localStorage`에 저장 → `/logs`에서 `Authorization: Bearer` 헤더로 로그·사용량 조회
- `/logs` 접근 시 토큰 없거나 만료(401)면 자동으로 `/login`으로 리다이렉트
- 비로그인 상태에서도 랜딩(`/`)은 노출, 질문 제출 시점에만 로그인 여부 확인 — 미로그인이면 안내 후 `/login`으로 이동
- 관리자 계정으로 로그인 시 헤더에 "관리자" 링크 노출 (`/me`로 권한 확인), `/admin`에서 계정 생성·권한 부여·로그인 정보 설정·비활성화·전체 이력 조회 가능
- `/admin` 계정 목록 — 관리자 계정이 상단에 고정 정렬되고, 비활성화된 계정은 기본적으로 숨겨짐(토글로 다시 확인 가능)
- 헤더에 로그아웃 버튼 — 인증 상태(`isLoggedIn`/`isAdmin`)와 로그아웃 로직을 `useAuth` 공용 훅으로 관리, `/`부터 순차 적용 중
- "FABLE" 텍스트 타이틀을 커스텀 워드마크 이미지(`fable-title.png`)로 교체
...
- [x] 대화 히스토리(멀티턴) UI
- [x] 로그인 게이트 + 관리자 페이지 (v0.6.0)
- [x] 관리자 페이지 UX 개선(계정 정렬/필터), 로그아웃, 워드마크 로고 반영 (v0.6.1)
- [ ] `logs`/`admin` 페이지도 `useAuth` 훅으로 완전 이전 (현재 `page.tsx`만 적용, 나머지는 로컬 로직 병행 중)
- [ ] 캐릭터 핸드오프 시각적 전환 효과
- [ ] apple-icon, OG 이미지 등 나머지 로고 자산 (favicon·타이틀 로고는 v0.6.1에서 반영 완료)